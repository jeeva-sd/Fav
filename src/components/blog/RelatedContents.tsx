...
const RelatedContents = () => {
  ...
  const contents: Posts[] = useMemo(() => {
    let contentArr: Posts[] = [];

    blogRoutes.forEach((_posts: Posts, index: number) => {
      const prevIndex = index ? index - 1 : blogRoutes.length - 1;
      const nextIndex = index && index !== blogRoutes.length - 1 ? index + 1 : 0;

      if (blogRoutes[index].path === pathname) {
        contentArr.push(blogRoutes[prevIndex]);
        contentArr.push(blogRoutes[nextIndex]);
      }
    });
          ...
          <div className="flex gap-5 w-full mb-6 lg:mb-8 md:grid-cols-2 cursor-pointer" key={index} onClick={() => push(content.path)}>
            <div className="w-full items-center bg-gray-50 rounded-lg shadow hover:shadow-lg transition-shadow sm:flex dark:bg-gray-800 dark:border-gray-700">
              <Image className="w-5/12 rounded-lg sm:rounded-none lg:flex md:flex hidden sm:rounded-l-lg h-full" width={200} height={200}
                ...
                alt="Bonnie Avatar" />
              <div className="p-5">
                <h3 className="text-xl font-bold tracking-normal dark:text-white text-indigo-600">
                  {content.title}
                </h3>
                <span className="text-gray-500 dark:text-gray-400 line-clamp-4">{content.description}</span>

                <p className="mt-3 mb-4 font-light text-gray-500 dark:text-gray-400 border rounded-md w-fit px-5 bg-white">
                  {content.type}
                </p>
                <ul className="space-x-4 sm:mt-0 hidden">
                  <li>
                    <a href="#" className="text-gray-500 hover:text-gray-900 dark:hover:text-white">
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 2`4 24" aria-hidden="true"><path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clip-rule="evenodd" /></svg>
                    </a>
                  </li>
                  <li>
                    <a href="#" className="text-gray-500 hover:text-gray-900 dark:hover:text-white">
